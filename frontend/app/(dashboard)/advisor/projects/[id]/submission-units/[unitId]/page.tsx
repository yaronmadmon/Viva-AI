...
const ADVISOR_TRANSITIONS: Record<string, string[]> = {
  ready_for_review: ["under_review"],
  under_review: ["approved", "revisions_required"],
  approved: ["locked"],
  locked: ["archived"],
};

export default function AdvisorSubmissionUnitDetailPage() {
  ...
  const nextStates = ADVISOR_TRANSITIONS[displayState] ?? [];
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <Link href={`/advisor/projects/${projectId}/submission-units`} className="hover:underline">Submission units</Link>
        <span>/</span>
